...
public class ThreeNodeGraphPattern implements Comparable<ThreeNodeGraphPattern>, Pattern {
	...
	public ThreeNodeGraphPattern(Triplet t) {
		if (t.numEdges == 2) {
			isWedge = true;
			StreamEdge edgeA = t.edgeA;//第一条边
			int sLabel = edgeA.getSrcLabel();//这条边源节点的lable
			int tLabel = edgeA.getDstLabel();//这条边终节点的lable

			StreamEdge edgeB = t.edgeB;//第二条边
	...
	public SubgraphType getType() {
		if (this.isWedge) {
			...
			return SubgraphType.TRIANGLE;//TRIANGLE
		}
	}
}
